# Modern Slavery PEC — Research & Resources

| Field | Value |
|---|---|
| **Jurisdiction** | UK |
| **Bucket** | Policy & Commentary |
| **Document Type** | Resource Collection |
| **Issuing Body** | Modern Slavery PEC |
| **Date Issued** | 2025 |
| **Status** | current |
| **Access** | public |

## Summary

UK independent research centre. Key publications include prosecution barriers analysis (Jun 2025: 467 CPS prosecutions, 355 convictions Jul 2023–Jun 2024), NRM data, ethical research guidance. Responds to House of Lords MSA Committee.

## Key Provisions

2025 prosecution barriers report; NRM analysis; policy briefs for practitioners

## Tags

`offence-elements` · `victimless-prosecution` · `evidence-corroboration`

## Source

[View source document](https://www.modernslaverypec.org/resources)

---

<!-- entry metadata (parsed by scripts — do not remove) -->
```yaml
title: "Modern Slavery PEC — Research & Resources"
jurisdiction: UK
bucket: policy-commentary
document_type: resource-collection
issuing_body: "Modern Slavery PEC"
date_issued: 2025
status: current
source_url: "https://www.modernslaverypec.org/resources"
access: public

summary: >
  UK independent research centre. Key publications include prosecution barriers analysis (Jun 2025: 467 CPS prosecutions, 355 convictions Jul 2023–Jun 2024), NRM data, ethical research guidance. Responds to House of Lords MSA Committee.

key_pinpoint: "2025 prosecution barriers report; NRM analysis; policy briefs for practitioners"

tags:
  - offence-elements
  - victimless-prosecution
  - evidence-corroboration

added_by: "Claude (import from resources.json)"
date_added: 2026-03-18
```
