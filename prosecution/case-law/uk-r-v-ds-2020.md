# R v. DS [2020] EWCA Crim 285

| Field | Value |
|---|---|
| **Jurisdiction** | UK |
| **Bucket** | Case Law |
| **Document Type** | Judgment / Opinion |
| **Issuing Body** | Court of Appeal (Criminal Division) |
| **Date Issued** | 2020-03-13 |
| **Status** | current |
| **Access** | public |

## Summary

Court of Appeal examined the application of the s. 45 Modern Slavery Act defence and the relationship between victim identification and non-punishment. Held that early identification of trafficking status is of paramount importance, and that a credible suspicion of trafficking must trigger a qualified assessment before any prosecution decision. The jury, not the judge on abuse of process, determines trafficking facts.

## Key Provisions

Paras. on s. 45 defence application, abuse of process arguments, and the nexus between victim identification and non-prosecution

## Tags

`non-punishment-link` · `indicators-screening` · `competent-authority-nrm` · `statutory-defence-s45` · `abuse-of-process`

## Source

[View source document](https://www.bailii.org/ew/cases/EWCA/Crim/2020/285.html)

**Local copy:** `docs/victim-identification/case-law/uk-r-v-ds-2020.html`

---

<!-- entry metadata (parsed by scripts — do not remove) -->
```yaml
title: "R v. DS [2020] EWCA Crim 285"
jurisdiction: UK
bucket: case-law
document_type: judgment-opinion
issuing_body: "Court of Appeal (Criminal Division)"
date_issued: 2020-03-13
status: current
source_url: "https://www.bailii.org/ew/cases/EWCA/Crim/2020/285.html"
access: public
local_file: "docs/victim-identification/case-law/uk-r-v-ds-2020.html"

summary: >
  Court of Appeal examined the application of the s. 45 Modern Slavery Act defence and
  the relationship between victim identification and non-punishment. Held that early
  identification of trafficking status is of paramount importance, and that a credible
  suspicion of trafficking must trigger a qualified assessment before any prosecution
  decision. The jury, not the judge on abuse of process, determines trafficking facts.

key_pinpoint: "Paras. on s. 45 defence application, abuse of process arguments, and the nexus between victim identification and non-prosecution"

tags:
  - non-punishment-link
  - indicators-screening
  - competent-authority-nrm
  - statutory-defence-s45
  - abuse-of-process

added_by: "Claude Code (initial build)"
date_added: 2026-03-09
```
