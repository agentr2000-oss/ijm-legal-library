# DOJ Human Trafficking Prosecution Unit (HTPU) Resources

| Field | Value |
|---|---|
| **Jurisdiction** | US |
| **Bucket** | Operational Guidance |
| **Document Type** | Guideline / SOP |
| **Issuing Body** | US Department of Justice |
| **Date Issued** | unknown |
| **Status** | current |
| **Access** | public |

## Summary

DOJ's HTPU within the Civil Rights Division provides federal prosecution guidance, coordinates multi-district investigations, and publishes best practices. Works with FBI, HSI, and US Attorneys' offices.

## Key Provisions

Prosecution of 18 USC §§1589–1592; coordination with FBI/HSI; restitution under §1593

## Tags

`offence-elements` · `sentencing-penalty` · `evidence-corroboration`

## Source

[View source document](https://www.justice.gov/humantrafficking)

---

<!-- entry metadata (parsed by scripts — do not remove) -->
```yaml
title: "DOJ Human Trafficking Prosecution Unit (HTPU) Resources"
jurisdiction: US
bucket: operational-guidance
document_type: guideline-sop
issuing_body: "US Department of Justice"
date_issued: unknown
status: current
source_url: "https://www.justice.gov/humantrafficking"
access: public

summary: >
  DOJ's HTPU within the Civil Rights Division provides federal prosecution guidance, coordinates multi-district investigations, and publishes best practices. Works with FBI, HSI, and US Attorneys' offices.

key_pinpoint: "Prosecution of 18 USC §§1589–1592; coordination with FBI/HSI; restitution under §1593"

tags:
  - offence-elements
  - sentencing-penalty
  - evidence-corroboration

added_by: "Claude (import from resources.json)"
date_added: 2026-03-18
```
