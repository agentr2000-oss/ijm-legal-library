# Australian Red Cross: Support for Trafficked People Program — Casework and Service Delivery Guidelines

| Field | Value |
|---|---|
| **Jurisdiction** | AU |
| **Bucket** | Operational Guidance |
| **Document Type** | Guideline / SOP |
| **Issuing Body** | Australian Red Cross |
| **Date Issued** | 2020-01-01 |
| **Status** | current |
| **Access** | public |

## Summary

Operational guidelines for the Australian Red Cross's delivery of the Support for Trafficked People Program (STPP), funded by the Department of Social Services. The Red Cross provides person-centred, individual case management through a dedicated caseworker, including assistance with accommodation, medical treatment, counselling, legal and migration advice referrals, skills development, and social support. Dependent children of trafficked persons may receive assistance with childcare, schooling, and medical support. The guidelines reflect a trauma-informed approach to post-identification victim support delivery.

## Key Provisions

STPP casework guidelines; individual case management framework; contact: 1800 113 015 / national_stpp@redcross.org.au

## Tags

`assistance-entitlements` · `referral-pathway` · `child-victim-identification`

## Source

[View source document](https://www.redcross.org.au/migration/support-for-trafficked-people/)

**Local copy:** `docs/victim-identification/operational-guidance/au-red-cross-support-guidelines.html`

---

<!-- entry metadata (parsed by scripts — do not remove) -->
```yaml
title: "Australian Red Cross: Support for Trafficked People Program — Casework and Service Delivery Guidelines"
jurisdiction: AU
bucket: operational-guidance
document_type: guideline-sop
issuing_body: "Australian Red Cross"
date_issued: 2020-01-01
status: current
source_url: "https://www.redcross.org.au/migration/support-for-trafficked-people/"
access: public
local_file: "docs/victim-identification/operational-guidance/au-red-cross-support-guidelines.html"

summary: >
  Operational guidelines for the Australian Red Cross's delivery of the Support for Trafficked People Program (STPP), funded by the Department of Social Services. The Red Cross provides person-centred, individual case management through a dedicated caseworker, including assistance with accommodation, medical treatment, counselling, legal and migration advice referrals, skills development, and social support. Dependent children of trafficked persons may receive assistance with childcare, schooling, and medical support. The guidelines reflect a trauma-informed approach to post-identification victim support delivery.

key_pinpoint: "STPP casework guidelines; individual case management framework; contact: 1800 113 015 / national_stpp@redcross.org.au"

tags:
  - assistance-entitlements
  - referral-pathway
  - child-victim-identification

added_by: "Claude Code (initial build)"
date_added: 2026-03-09
```
