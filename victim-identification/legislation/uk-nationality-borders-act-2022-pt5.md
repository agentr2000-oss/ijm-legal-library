# Nationality and Borders Act 2022 (Part 5 — Modern Slavery)

| Field | Value |
|---|---|
| **Jurisdiction** | UK |
| **Bucket** | Legislation |
| **Document Type** | Act / Statute |
| **Issuing Body** | UK Parliament |
| **Date Issued** | 2022-04-28 |
| **Status** | current |
| **Access** | public |

## Summary

Reforms the National Referral Mechanism process by introducing trafficking and slavery information notices, statutory time limits for conclusive grounds decisions, and a revised definition of victim status. Part 5 reshapes victim identification procedures, including the reasonable grounds threshold and disqualification criteria for certain individuals.

## Key Provisions

ss. 57–62 (trafficking information notices, identification of potential victims, recovery periods, disqualification from protection)

## Victim Identification Tags

`competent-authority-nrm` · `definition-scope` · `referral-pathway` · `reflection-period-temporary-stay`

## Source

[View source document](https://www.legislation.gov.uk/ukpga/2022/36/part/5)

**Local copy:** `docs/victim-identification/legislation/uk-nationality-borders-act-2022-pt5.html`

---

<!-- entry metadata (parsed by scripts — do not remove) -->
```yaml
title: "Nationality and Borders Act 2022 (Part 5 — Modern Slavery)"
jurisdiction: UK
bucket: legislation
document_type: act-statute
issuing_body: "UK Parliament"
date_issued: 2022-04-28
status: current
source_url: "https://www.legislation.gov.uk/ukpga/2022/36/part/5"
access: public
local_file: "docs/victim-identification/legislation/uk-nationality-borders-act-2022-pt5.html"

summary: >
  Reforms the National Referral Mechanism process by introducing trafficking and slavery
  information notices, statutory time limits for conclusive grounds decisions, and a revised
  definition of victim status. Part 5 reshapes victim identification procedures, including
  the reasonable grounds threshold and disqualification criteria for certain individuals.

key_pinpoint: "ss. 57–62 (trafficking information notices, identification of potential victims, recovery periods, disqualification from protection)"

victim_id_tags:
  - competent-authority-nrm
  - definition-scope
  - referral-pathway
  - reflection-period-temporary-stay

added_by: "Claude Code (initial build)"
date_added: 2026-03-09
```
